...
// ─── SUNDAY PLAN — PURE CORE ─────────────────────────────────────────────────
// Prompt construction, response→planData mapping and the plan-addresses-priority
// matcher, lifted verbatim out of planGen.js. No Firestore and no ai.js import,
// so a Cloud Function can build byte-identical prompts and planData from the
// same inputs. planGen.js keeps the Firestore writes.
//
...
export const SUNDAY_PLAN_MAX_TOKENS = 6000;

// ── buildSundayPlanPrompt ────────────────────────────────────────────────────
// Pure. Returns { system, prompt, maxTokens }.
//
// `metrics`, `loadNotes` and `thisWeekStart` default to exactly what planGen.js
// computed inline, so a caller that already has them (planGen needs `metrics`
// for planData) can pass them rather than recompute.
export function buildSundayPlanPrompt({
  profile,
  weekLogs = [],
  sessionHistory = [],
  wellbeing = [],
  ...
  loadNotes = getACWRContext(metrics.acwr, tournament, sessionTime),
...
Return ONLY a raw JSON object. Do NOT wrap in markdown code fences. Do NOT include \`\`\`json or \`\`\` anywhere in your response. Start your response with { and end with }.`;
  ...
  return { system, prompt, maxTokens: SUNDAY_PLAN_MAX_TOKENS };
}

// ── toPlanData ───────────────────────────────────────────────────────────────
// Pure. Maps the model's exercises schema onto the existing plan format (so all
// display logic is unchanged) and assembles the plans/current document.
// `metrics` is the calculateMetrics result — PlanScreen renders plan.metrics.
export function toPlanData(parsed, ctx, generatedAt, metrics = null) {
  const p = parsed || {};

  // Map new exercises schema → existing plan format so all display logic is unchanged
  const plan = (p.exercises || []).map(ex => ({
    ...ex,
  ...
  return {
    ...
    sessionDuration: p.sessionDuration ?? null,
    ...
    coachNote:       p.coachNote       ?? null,
    athleteNote:     p.athleteNote     ?? null,
...
// Pure. The deferred-priority labels today's exercises actually address, in the
// order the original loop produced them (repeats included — the second resolve
// of an already-resolved area is a no-op on the applier side).
