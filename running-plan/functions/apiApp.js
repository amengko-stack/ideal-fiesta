// Single source of truth for the running-plan API. Used by:
//   - functions/index.js  (production Cloud Function)
//   - ../dev-server.js     (local dev + Playwright E2E)
// CommonJS so it loads unchanged inside the Firebase Functions runtime.
// Node 20+ provides a global fetch, so no node-fetch dependency is needed.
...
// Builds and returns the configured Express app. Reads process.env lazily on
// each request handler, so the same instance works in dev and in Functions.
function createApiApp() {
    ...
    try {
      const upstream = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: Math.min(Number(max_tokens) || 4000, 6000),
          system: system || "",
          messages,
        }),
      });
  ...
  // ─── WHOOP ───────────────────────────────────────────────────────────────────
  const whoopMock = () => process.env.WHOOP_MOCK === "1";
  ...
  app.get("/api/whoop/config", (req, res) => {
    res.json({
      enabled: whoopConfigured() || whoopMock(),
      mock: whoopMock(),
